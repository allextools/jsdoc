...
var testForAllex = require('jsdoc/util/templateAllexHelper');
...
function buildLink(longname, linkText, options) {
    ...
    allextest = testForAllex(stripped);
    if (allextest) {
      fileUrl = allextest.url;
      text = linkText || allextest.linkText;
    }
    else if ( hasUrlPrefix(stripped) ) {
        fileUrl = stripped;
